...
func main() {
	...
	if vaultClient != nil {
		...
		// 从 Vault 覆盖 RabbitMQ 配置
		if secrets, err := vaultclient.GetKVSecret(vaultClient, "secret/data/rabbitmq"); err == nil {
			vaultclient.OverlayString(secrets, "host", &cfg.RabbitMQ.Host)
			vaultclient.OverlayString(secrets, "user", &cfg.RabbitMQ.User)
			vaultclient.OverlayString(secrets, "password", &cfg.RabbitMQ.Password)
		}
		// 从 Vault 覆盖 JWT 密钥
